...
### 概要
バックエンドの CORS 設定を `*` (全許可) からホワイトリスト方式に変更。`middleware/cors.go` を新規作成し、`localhost:3000`, `*.vercel.app`, `wyze-system.com` のみ許可。Cloud Run に再デプロイし、CORS 動作確認完了。

### 詳細

#### 1. CORS ミドルウェア実装
- **[Claude]** `backend/internal/middleware/cors.go` を新規作成
  - `allowedOrigins` リスト: `http://localhost:3000`, `https://wyze-system.com`
  - `*.vercel.app` パターンマッチ (`HasPrefix("https://")` + `HasSuffix(".vercel.app")`)
  - `AllowCredentials: true`, `Vary: Origin` ヘッダー付与
  - 許可されないオリジンには CORS ヘッダーを返さない

#### 2. main.go のインライン CORS 削除
- **[Claude]** `backend/cmd/server/main.go`
  - `corsMiddleware()` 関数を削除
  - `r.Use(corsMiddleware())` → `r.Use(middleware.CORS())` に変更
  - 不要になった `"net/http"` import を削除

#### 3. デプロイ & 疎通確認
- **[Claude]** Cloud Build でビルド & プッシュ → Cloud Run 再デプロイ (revision: `backend-00002-vpl`)
- **[Claude]** CORS preflight 確認:
  - `Origin: http://localhost:3000` → `Access-Control-Allow-Origin: http://localhost:3000` ✅
  - `Origin: https://test-app.vercel.app` → `Access-Control-Allow-Origin: https://test-app.vercel.app` ✅
  - `Origin: https://evil-site.com` → CORS ヘッダーなし (拒否) ✅

#### 4. ドキュメント更新
- **[Claude]** `docs/architecture.md` — CORS 設定セクション追記、申し送り事項更新
- **[Claude]** `docs/change-log.md` — 本エントリ

### 変更ファイル
- `backend/internal/middleware/cors.go` — 新規作成
- `backend/cmd/server/main.go` — CORS ミドルウェア差し替え
- `docs/architecture.md` — CORS 設定追記
- `docs/change-log.md` — 本エントリ追加
...
### 概要
Phase 4: CI/CD パイプライン (CD部分) を構築。`develop` ブランチへの push をトリガーに、マイグレーション → ビルド → Cloud Run デプロイ → ヘルスチェックを自動実行するワークフローを作成。

### 詳細

#### 1. GitHub Actions CD ワークフロー作成
- **[Claude]** `.github/workflows/cd-staging.yml` を新規作成
  - トリガー: `develop` ブランチ push (`backend/**` パス変更時)
  - フロー: Checkout → GCP認証 → Docker Build & Push → マイグレーション → Cloud Run デプロイ → ヘルスチェック
  - イメージタグ: `<commit SHA>` + `latest` の2タグ方式

#### 2. マイグレーション自動化
- **[Claude]** Cloud SQL Proxy (v2.14.3) を GitHub Actions 内で起動し、既存の `cmd/migrate/main.go up` を実行
- **[Claude]** マイグレーション失敗時はデプロイを中断する安全機構を実装

#### 3. GCPサービスアカウント作成
- **[Claude]** `github-actions-deploy@wyze-develop-staging.iam.gserviceaccount.com` を作成
- **[Claude]** 最小権限を付与: `run.developer`, `artifactregistry.writer`, `storage.objectViewer`, `secretmanager.secretAccessor`, `cloudsql.client`, `iam.serviceAccountUser`
- **[Claude]** SAキーを `/tmp/gcp-sa-key.json` に出力

#### 4. CORS確認
- **[Claude]** `backend/cmd/server/main.go` の CORS は `*` (全許可) — ステージングとして許容、本番移行時に制限予定

#### 5. ドキュメント更新
- **[Claude]** `docs/ci-cd.md` — CD Staging セクション追記 (フロー図、GitHub Secrets登録手順、SA情報)
- **[Claude]** `docs/Task.md` — Phase 4 タスク更新
- **[Claude]** `docs/change-log.md` — 本エントリ

### マスター作業が必要な項目
1. **GitHub Secrets 登録** (Settings → Secrets and variables → Actions):
   - `GCP_SA_KEY`: `/tmp/gcp-sa-key.json` の内容を貼り付け
   - `GCP_PROJECT_ID`: `wyze-develop-staging`
   - `DATABASE_URL_TCP`: `postgres://wyze_app:<password>@127.0.0.1:5432/wyze_db?sslmode=disable`
     - パスワードは `gcloud secrets versions access latest --secret=DATABASE_URL` で確認可能
2. 登録後、`/tmp/gcp-sa-key.json` を削除

### 変更ファイル
- `.github/workflows/cd-staging.yml` — 新規作成
- `docs/ci-cd.md` — CD Staging 追記
- `docs/Task.md` — Phase 4 更新
- `docs/change-log.md` — 本エントリ追加
...
### 概要
Phase 3: GCPステージング環境 (`wyze-develop-staging`) の構築完了。Cloud SQL, Secret Manager, Cloud Run を `us-east1` リージョンに構築し、バックエンドAPIのデプロイと疎通確認まで実施。

### 詳細

#### 1. GCPプロジェクト準備
- **[Claude]** API有効化: `run.googleapis.com`, `sqladmin.googleapis.com`, `vpcaccess.googleapis.com`, `servicenetworking.googleapis.com`
- **[Claude]** リージョンを `us-east1` (Always Free対象) に設定
- **[Claude]** Artifact Registry リポジトリ `web-system-pj` (Docker format, `us-east1`) を作成

#### 2. VPCネットワーク & Cloud SQL構築
- **[Claude]** `default` VPC にプライベートサービスアクセスを構成 (IP範囲予約 `google-managed-services-default` + VPCピアリング)
- **[Claude]** サーバーレスVPCコネクタ `vpc-con-us-east1` を作成 (`10.8.0.0/28`, e2-micro, 2-3 instances)
- **[Claude]** Cloud SQL インスタンス `wyze-staging-db` を作成 (PostgreSQL 16, `db-f1-micro`, Enterprise edition, Private IP: `10.26.0.3`, HDD 10GB)
- **[Claude]** DB `wyze_db` およびアプリユーザー `wyze_app` を作成

#### 3. Secret Manager設定
- **[Claude]** `DATABASE_URL` シークレット作成 (Cloud SQL Unix Socket形式: `postgres://wyze_app:***@/wyze_db?host=/cloudsql/wyze-develop-staging:us-east1:wyze-staging-db`)
- **[Claude]** `JWT_SECRET` シークレット作成 (48文字ランダム)
- **[Claude]** Cloud Run デフォルトSA (`611370943102-compute@developer.gserviceaccount.com`) に `roles/secretmanager.secretAccessor` を付与

#### 4. Cloud Runデプロイ
- **[Claude]** `backend/Dockerfile` を Go用マルチステージビルドに修正 (旧: Next.js用)
- **[Claude]** Cloud Build でビルド & Artifact Registry へプッシュ成功
- **[Claude]** Cloud Run サービス `backend` をデプロイ (VPCコネクタ接続, Secret Managerマウント, Cloud SQL インスタンス接続, `GIN_MODE=release`)
- **[Claude]** 疎通確認: `https://backend-611370943102.us-east1.run.app/health` → `200 OK {"status":"ok"}`

#### 5. ドキュメント更新
- **[Claude]** `docs/Task.md` — Phase 3 の全タスクを完了済みに更新、構築リソース情報を記録
- **[Claude]** `docs/architecture.md` — GCPステージング環境構成図 (mermaid)、リソース一覧、接続フロー、本番移行申し送り事項を追記
- **[Claude]** `docs/change-log.md` — 本エントリ

### 構築したリソース
| リソース | 名前 | 備考 |
|---------|------|------|
| Artifact Registry | `web-system-pj` | Docker, us-east1 |
| VPC Connector | `vpc-con-us-east1` | 10.8.0.0/28 |
| Cloud SQL | `wyze-staging-db` | PostgreSQL 16, db-f1-micro, Private IP |
| Secret Manager | `DATABASE_URL`, `JWT_SECRET` | — |
| Cloud Run | `backend` | https://backend-611370943102.us-east1.run.app |

### 変更ファイル
- `backend/Dockerfile` — Go用マルチステージビルドに書き換え
- `docs/Task.md` — Phase 3 タスク完了更新
- `docs/architecture.md` — GCPステージング構成追記
- `docs/change-log.md` — 本エントリ追加
