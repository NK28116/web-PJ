...
## wyge-system.comが限定公開されている設定になっているか
まだ開発段階なので開発者と起票者のみのアドレスを知っている人だけがアクセスできるようになっていてほしい


## stg.wyze-system.com環境下でのテスト結果

developから切ったfix/stagingDeployProblemからfix/stagingDeployProblem-section1を切って解決したらfix/stagingDeployProblemにマージ
developから切ったfix/stagingDeployProblemからfix/stagingDeployProblem-section2を切って解決したらfix/stagingDeployProblemにマージ
developから切ったfix/stagingDeployProblemからfix/stagingDeployProblem-section3を切って解決したらfix/stagingDeployProblemにマージ
のようにして1つずつ解決していく
...
#### 新規登録画面
##### 1. 入力状態がわかりにくい
  - 入力待機状態になっていることやテキストカーソルが表示されていないので入力位置がわかりにくい
    - フォーカスが外れているときのテキストボックスの背景は`text-gray-900`,入力中は`text-white`で入力位置はcursor-bar-blinkingで明示する
  
##### 2. 入力が手間取る
- 認証番号の入力でコピーアンドペーストが効かず入力に手間取る
- 認証番号はメール動作をまだ設定してないので，画面下部に` gcloud run services logs read backend --region=us-east1 --limit=20
`から`【Verification Code】 email={$メールアドレス} code={$コード} `をフィルターして表示させる

##### 3. 画面外が白いまま
- 画面外の背景が初期のwhiteのままなのでコンテンツに合わせる
...
```
##### 2. アカウント削除の動線がない
- 店舗一覧ブロックの下に「退会の申し込みはサポートヘルプから」の文言を追加

#### 投稿 https://stg.wyze-system.com/post
##### 1.未連携なのに表示されている
- 未連携なのに表示されている
  - モックの外し忘れ
    - 未連携の時は「Google Businessアカウントと連携してくださいと表示される」

#### お支払い情報(https://stg.wyze-system.com/billing)
##### 1. カード情報入力欄使用不可
- 「カードを登録する」ボタンを押しても「クレジットカードを登録してください」表示が外れない
##### 2. お支払い履歴のモック外し忘れ
- 存在しないはずのお支払い履歴がある
...
- 存在しない
  - おそらく作り忘れ
    - 初期状態:未入力のテキストフォーム,送信ボタン,「アカウント削除依頼の方は"アカウント削除依頼\nユーザー名:\nメールアドレス:\n"と入力の上送信ボタンを押してください」
    - 入力中からフォーム外をタップで入力状態解除
    - 送信ボタンを押したら「お問い合わせありがとうございます」と表示される

#### 店舗アカウント設定　https://stg.wyze-system.com/account
- 店舗プロフィール情報
  - Google連携してないのに表示されている
    - モックの外し忘れ
- オーナープロフィール情報
  - 新規登録時に使用したユーザーネーム使用したい
- 通知設定
  - 初期状態では全てオフ
